class IssueModal {
  ...
  selectIssueType(issueType) {
    ...
    const selector = `[data-testid="select-option:${issueType}"]`;
    cy.log(`Using selector: ${selector}`);
    cy.get(selector).trigger('mouseover').trigger('click');
    const iconSelector = `[data-testid="icon:${issueType.toLowerCase()}"]`;
    ...
    if (issueType === 'Story') {
      ...
      cy.get(this.taskIcon).should('be.visible');
    } else {
  ...
  ensureIssueIsCreated(expectedAmountIssues, issueDetails) {
    ...
    cy.get(this.backlogList)
      ...
      .within(() => {
        ...
        cy.get(
          `[data-testid="icon:${issueDetails.icon.toLowerCase()}"]`
        ).should('be.visible');
