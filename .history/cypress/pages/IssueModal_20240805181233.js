class IssueModal {
  ...
  selectPriority(priority) {
    ...
    cy.get(`[data-testid="select-option:${priority}"]`)
      .should('be.visible')
      .then(() => {
        cy.log(`Priority "${priority}" selected successfully`);
      })
      .catch(() => {
        cy.log(
          `Priority "${priority}" not found in dropdown. Setting default priority.`
        );
        cy.get('[data-testid="select-option:Medium"]').click(); // Example: Setting a default priority
      });
  }
