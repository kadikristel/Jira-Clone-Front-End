    ...
    cy.get(this.issueComment)
      ...
      .then(($comment) => {
        if ($comment.length > 0) {
          ...
          // Check that the comment element itself is gone from the DOM
          cy.get(this.issueComment).should('not.exist');
        } else {
          // Log a message that the comment was not found
          cy.log(
            `Comment with text "${commentText}" does not exist, skipping deletion.`
          );
        }
      });
  }
