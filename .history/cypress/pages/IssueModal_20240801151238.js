class IssueModal {
  ...
  selectIssueType(issueType, iconTestId) {
    cy.get(this.issueType).click('bottomRight');
    cy.get(`[data-testid="select-option:${issueType}"]`)
      .trigger('mouseover')
      .trigger('click');
    cy.get(`[data-testid="icon:${iconTestId}"]`).should('be.visible');
  }
  ...
  validateIconIsDefault(iconTestId) {
    cy.get(`[data-testid="icon:${iconTestId}"]`)
      .should('be.visible')
      .and('not.have.class', 'story')
      .and('not.have.class', 'bug')
      .and('not.have.class', 'task');
  }
