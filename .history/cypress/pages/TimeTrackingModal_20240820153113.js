class TimeTrackingModal {
  ...
  clickDoneButton() {
    cy.contains('button', this.doneButtonName).click().should('not.exist');
  }
  ...
  removeTimeSpent(hours) {
    cy.get(this.inputPlaceholderNumber).first().clear().wait(1000);

    cy.get(this.stopwatchIcon).parent().should('contain', 'No time logged');
  }
