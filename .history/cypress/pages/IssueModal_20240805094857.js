class IssueModal {
  ...
  selectIssueType(issueType) {
    ...
    // Check if the desired issue type option exists in the dropdown
    cy.get('body').then(($body) => {
      if ($body.find(`[data-testid="select-option:${issueType}"]`).length) {
        ...
        // Verify the selected issue type text is visible
        cy.get(this.issueType).should('contain', issueType);
      } else {
        // If the option does not exist, log an error
        cy.log(`Issue type ${issueType} not found in the dropdown`);
        throw new Error(`Issue type ${issueType} not found in the dropdown`);
      }
    });
  }
