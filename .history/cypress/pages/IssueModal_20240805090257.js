class IssueModal {
    ...
    cy.get(this.issueType)
      ...
      .then(() => {
        cy.get(`[data-testid="select-option:${issueType}"]`)
          .should('be.visible')
          .trigger('mouseover')
          .wait(300) // Adjust wait time as needed
          .trigger('click');
      });
  }
  ...
  selectPriority(priority) {
    ...
    cy.get('[data-testid="select:priority"]')
      .should('have.class', 'jira-select-option-is-active') // Assuming this class indicates active selection
      .within(() => {
  ...
  ensureIssueIsCreated(expectedAmountIssues, issueDetails) {
    ...
    cy.get(this.backlogList)
      ...
      .within(() => {
        ...
        cy.get(this.priorityOption).should('have.text', issueDetails.priority);

        cy.get('[data-testid="select:priority"]')
          .should('be.visible')
          .within(() => {
            cy.contains(issueDetails.priority).should('be.visible');
          });
      });
  }
