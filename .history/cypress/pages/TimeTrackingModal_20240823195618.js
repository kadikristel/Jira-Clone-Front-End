class TimeTrackingModal {
  ...
  selectIssueType(issueType) {
    ...
    cy.get(this.allIssueType).eq(`${issueType}`);
  }

  selectPriority(priorityOption) {
    cy.get(this.priorityDropdown).click();
    cy.get(this.allPriorityOptions).eq(`${priorityOption}`).click();
  }

  selectReporter(reporterName) {
    cy.get(this.reporter).click().type(reporterName);
  }

  selectAssignee(assigneeName) {
    cy.get(this.assignee).click().type(assigneeName);
  }
