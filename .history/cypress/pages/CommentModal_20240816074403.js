class CommentModal {
  ...
  createComment({ newComment }) {
    this.getIssueDetailsModal().within(() => {
      cy.contains('div', 'Add a comment...').scrollIntoView().click();
      cy.get('.comment-textarea').type(newComment);
      cy.get('.save-button').click();
      cy.get('.issue-comment').should('contain', newComment);
    });
  }

  editComment(newCommentText) {
    cy.get('.issue-comment').contains('Edit').click();
    cy.get('.comment-textarea').clear().type(newCommentText);
    cy.get('.save-button').click();
    cy.get('.issue-comment').should('contain', newCommentText);
  }
