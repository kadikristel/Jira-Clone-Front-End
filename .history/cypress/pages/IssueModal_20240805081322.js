class IssueModal {
  ...
  selectIssueType(issueType) {
    ...
    const optionSelector = `[data-testid="select-option:${issueType}"]`;
    cy.get(optionSelector)
      .should('be.visible')
      .click()
      .then(() => {
        cy.get(this.issueType).within(() => {
          cy.get(`[data-testid="icon:${issueType.toLowerCase()}"]`)
            .should('be.visible')
            .then(() => {
              cy.log(`${issueType} icon is now visible`);
            });
        });
