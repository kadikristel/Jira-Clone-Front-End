class IssueModal {
  ...
  selectIssueType(issueType, iconId) {
    cy.get(this.issueType).click('bottomRight');
    cy.get(`[data-testid="select-option:${issueType}"]`)
      .trigger('mouseover')
      .trigger('click');
    cy.get(`[data-testid="icon:${iconId}"]`).should('be.visible');
  }
  ...
  validateIconIsDefault(iconTestId) {
    cy.get(`[data-testid="icon:${iconTestId}"]`)
      .should('be.visible')
      .and('not.have.class', 'story') // Ensure it's not a story
      .and('not.have.class', 'other-class'); // Add any other classes to check for
  }
  ...
  ensureIssueIsCreated(expectedAmountIssues, issueDetails) {
    ...
    cy.get(this.backlogList)
      ...
      .within(() => {
        ...
        cy.get(`[data-testid="icon:${issueDetails.iconId}"]`).should(
          'be.visible'
        );
      });
  }
