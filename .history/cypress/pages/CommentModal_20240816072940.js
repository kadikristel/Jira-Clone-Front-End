class CommentModal {
  ...
  editComment(newCommentText, commentSelector = null) {
    if (commentSelector) {
      cy.get(commentSelector).contains('Edit').click();
    } else {
      cy.get(CommentModal.issueComment).first().contains('Edit').click();
    }

    cy.get(CommentModal.commentTextArea).clear().type(newCommentText);
    cy.contains('button', CommentModal.saveButtonName).click();
  }
