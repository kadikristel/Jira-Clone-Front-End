...
describe('Time estimation functionality for a new issue', () => {
  beforeEach(() => {
    cy.visit('/');
    cy.url()
  ...
  it('Should add, edit and remove time estimation to a created issue', () => {
    const inputHours = 10;
  ...
  describe('Time estimation functionality for an existing issue', () => {
    beforeEach(() => {
      cy.visit('/');
      cy.url()
        .should('eq', `${Cypress.env('baseUrl')}project`)
    ...
    it.only('Should add, edit and remove time estimation to an issue', () => {
      const inputHours = 10;
      const editedHours = 16;
      const initialWidth = '100';
