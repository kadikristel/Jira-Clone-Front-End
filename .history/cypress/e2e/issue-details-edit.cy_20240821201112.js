...
describe('JavaScript tasks 1&2', () => {
  ...
  it.only('Should check Priority dropdown values for existing issue', () => {
    ...
    .then((selectedPriorityOption) => {
      const initialPriority = selectedPriorityOption.trim();

    cy.get('[data-testid="select:priority"]').click('bottomRight');

    if (optionText !== initialPriority) {
            actualPriorities.push(optionText);
          }
    ...
    cy.get('[data-testid="select:priority"]')
      ...
      .then((selectedPriorityOption) => {
        ...
        cy.get('[data-testid^="select-option:"]')
          .each(($option) => {
            const initialPriority = $option.text().trim();
            actualPriorities.push(initialPriority);
