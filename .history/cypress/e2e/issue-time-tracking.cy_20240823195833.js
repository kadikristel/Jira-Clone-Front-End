...
describe('Time tracking functionality for new issue', () => {
  ...
  it('Should add and remove time logging for the issue', () => {
    ...
    TimeTrackingModal.getIssueModal().within(() => {
      TimeTrackingModal.selectIssueType(1).click();
      cy.get(TimeTrackingModal.descriptionField).type(
        issueData.randomDescription
      );
      cy.get(TimeTrackingModal.title).type(issueTitle);
      TimeTrackingModal.selectReporter('Pickle Rick');
      TimeTrackingModal.selectAssignee('Baby Yoda');
      TimeTrackingModal.selectPriority(3).click();
      TimeTrackingModal.clickCreateIssueButton();
    });
