...
describe('Issue create', () => {
  ...
  it.only('Should create an issue and validate it successfully', () => {
    cy.get(this.issueModal).within(() => {
      cy.get(this.descriptionField).type('TEST_DESCRIPTION');
      cy.get(this.descriptionField).should('have.text', 'TEST_DESCRIPTION');
      cy.get(this.title).type('TEST_TITLE');
      cy.get(this.title).should('have.value', 'TEST_TITLE');
      selectIssueType(this.selectOptionStory);
      cy.get(this.selectOptionStory)
        .wait(1000)
        .trigger('mouseover')
        .trigger('click');
      cy.get(this.storyIcon).should('be.visible');
      selectReporter(this.reporterBabyYoda);
      selectingAssignee(this.assigneePickleRick);
      createIssue();
    });
