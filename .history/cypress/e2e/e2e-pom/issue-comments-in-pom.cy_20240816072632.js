...
describe('Issue comments creating, editing and deleting using POM approach', () => {
  ...
  it.only('Should create, edit and delete a comment', () => {
    const firstCommentSelector = CommentModal.issueComment.first();

    CommentModal.createComment({
      newComment: commentData.newComment,
    });
    CommentModal.editComment(commentData.editedComment, firstCommentSelector);
    CommentModal.deleteComment(commentData.editedComment);
  });
});
