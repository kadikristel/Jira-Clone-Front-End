...
const selectors = {
  ...
  selectOptionTask: '[data-testid="select-option:Task]',
  ...
  babyYodaAvatar: '[data-testid="avatar: Baby Yoda"]',
...
describe('Issue create', () => {
  ...
  const issueData = {
    randomTitle: faker.commerce.productName(),
    randomDescription: faker.internet.description(),
  };

  it.only('Should create a custom issue and validate it successfully using Random Data Plugin', () => {
    cy.get(selectors.modalIssueCreate).within(() => {
      cy.get(selectors.descriptionInputField).type(randomDescription);
      cy.get(selectors.descriptionInputField).should('have.length', 5);
      cy.get(selectors.inputNameTitle).type(randomTitle);
      cy.get(selectors.inputNameTitle).should('have.length', 1);
      selectTypeDropdown();
    ...
    cy.get(selectors.boardListBackLog)
      ...
      .within(() => {
        cy.get(selectors.listIssue)
          ...
          .siblings()
          .within(() => {
            cy.get(selectors.babyYodaAvatar).should('be.visible');
            cy.get(selectors.taskIcon).should('be.visible');
          });
      });
  });
