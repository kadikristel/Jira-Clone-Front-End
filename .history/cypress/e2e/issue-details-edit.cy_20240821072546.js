...
describe('Priority dropdown', () => {
  ...
  it.only('Should check Priority dropdown values for existing issue', () => {
    ...
    cy.get('[data-testid="select:priority" ."sc-cpmLhU dCLQTz"]').click(
      'bottomRight'
    );
    cy.get('[data-testid="select:priority"]')
      .invoke('text')
      .then((text) => {
        priorityValues.push(text);
      });

    cy.get('[data-testid="select:priority"]').click('bottomRight');

    cy.get('[data-testid="icon:chevron-down"]')
      .each(($li) => {
      ...
      .then(() => {
        ...
        expect(priorityValues).to.deep.equal(expectedPriorities);
      });
  });
});
