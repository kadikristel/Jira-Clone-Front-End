...
describe('Issue comments creating, editing and deleting', () => {
  ...
  it.only('Should create a comment successfully', () => {
    CommentModal.getIssueDetailsModal().within(() => {
      ...
      cy.get(CommentModal.commentTextArea).type(newComment);
      cy.get(CommentModal.commentTextArea).should('have.text', newComment);
      cy.contains(clickSaveButton).should('not.exist');
      cy.contains(CommentModal.addCommentArea).should('exist');
      cy.get(CommentModal.issueComment).should('contain', newComment);
    });
  });
