...
describe('Time estimation functionality for new issue', () => {
  ...
  it.only('Should add, edit and remove the estimation of the issue', () => {
    ...
    cy.get('[data-testid="icon:stopwatch"]')
      .parent()
      .within(() => {
        cy.get('div').eq(0).should('contain', 'No time logged');
      });

    cy.get('input[placeholder="Number"]').first().clear().type('10{enter}');
    ...
    cy.get('[data-testid="modal:issue-details"]').within(() => {
      cy.get('input[placeholder="Number"]')
        ...
        cy.get('[data-testid="icon:stopwatch"]').parent().within(() => {
      cy.get('div').eq(1).should('contain', '10h estimated');
    });
  });
});
