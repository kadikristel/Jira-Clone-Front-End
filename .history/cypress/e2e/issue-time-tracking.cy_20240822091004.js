...
describe('Time estimation functionality for new issue', () => {
  ...
  it('Should add, edit and remove the time estimation for the issue', () => {
    ...
    cy.get(TimeTrackingModal.issueDetailsModal).within(() => {
      cy.get(TimeTrackingModal.stopwatchIcon).parent();
      should('be.visible').and('contain', 'No time logged');
    });
    ...
    cy.get(TimeTrackingModal.stopwatchIcon)
      .parent()
      .should('be.visible')
      .and('not.contain', `${editedHours}h estimated`);
  });
});
...
describe('Time estimation and tracking functionalities for existing issue', () => {
  ...
  it('Should add, edit and remove the time estimation and tracking for the issue', () => {
    ...
    cy.get(TimeTrackingModal.issueDetailsModal).within(() => {
      ...
      cy.get(TimeTrackingModal.stopwatchIcon).parent();
      should('be.visible').and('contain', `${editedHours}h estimated`);
    });
    ...
    cy.get(TimeTrackingModal.stopwatchIcon).parent();
    should('be.visible').and('not.contain', `${editedHours}h estimated`);
    ...
    cy.get(TimeTrackingModal.stopwatchIcon).parent();
    should('be.visible').and('contain', `${timeSpent}h logged`);
