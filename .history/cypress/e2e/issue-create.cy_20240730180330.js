import {
  selectTypeDropdown,
  selectPriority,
  selectingReporterPickleRickDropdown,
  selectingReporterBabyYodaDropdown,
  selectingAssigneeLordGabenDropdown,
  clickingCreateIssueButton,
  selectingAssigneePickleRickDropdown,
} from "../pages/functions.js";

describe("Issue create", () => {
  beforeEach(() => {
    cy.visit("/");
    cy.url()
      .should("eq", `${Cypress.env("baseUrl")}project`)
      .then((url) => {
        // System will already open issue creating modal in beforeEach block
        cy.visit(url + "/board?modal-issue-create=true");
      });
  });

  const modalIssueCreate = "[data-testid='modal:issue-create']";
  const inputNameTitle = "input[name='title']";
  const descriptionInputField = ".ql-editor";
  const selectOptionBug = "[data-testid='select-option:Bug']";
  const selectOptionStory = '[data-testid="select-option:Story"]';
  const bugIcon = "[data-testid='icon:bug']";
  const successMessage = "Issue has been successfully created.";
  const boardListBackLog = "[data-testid='board-list:backlog']";
  const listIssue = "[data-testid='list-issue']";
  const lordGabenAvatar = "[data-testid='avatar:Lord Gaben']";
  const priorityOptionHighest = "[data-testid='select-option:Highest']";
  const pickleRickAvatar = '[data-testid="avatar:Pickle Rick"]';
  const storyIcon = '[data-testid="icon:story"]';

  it("Should create an issue and validate it successfully", () => {
    // System finds modal for creating issue and does next steps inside of it
    cy.get(modalIssueCreate).within(() => {
      // Type value to description input field
      cy.get(descriptionInputField).type("TEST_DESCRIPTION");
      cy.get(descriptionInputField).should("have.text", "TEST_DESCRIPTION");

      // Type value to title input field
      // Order of filling in the fields is first description, then title on purpose
      // Otherwise filling title first sometimes doesn't work due to web page implementation
      cy.get(inputNameTitle).type("TEST_TITLE");
      cy.get(inputNameTitle).should("have.value", "TEST_TITLE");

      // Open issue type dropdown and choose Story
      selectTypeDropdown();
      cy.get(selectOptionStory)
        .wait(1000)
        .trigger("mouseover")
        .trigger("click");
      cy.get(storyIcon).should("be.visible");

      // Select Baby Yoda from reporter dropdown
      selectingReporterBabyYodaDropdown();

      // Select Pickle Rick from assignee dropdown
      selectingAssigneePickleRickDropdown();

      // Click on button "Create issue"
      clickingCreateIssueButton();
    });

    // Assert that modal window is closed and successful message is visible
    cy.get(modalIssueCreate).should("not.exist");
    cy.contains(successMessage).should("be.visible");

    // Reload the page to be able to see recently created issue
    // Assert that successful message has dissappeared after the reload
    cy.reload();
    cy.contains(successMessage).should("not.exist");

    // Assert than only one list with name Backlog is visible and do steps inside of it
    cy.get(boardListBackLog)
      .should("be.visible")
      .and("have.length", 1)
      .within(() => {
        // Assert that this list contains 5 issues and first element with tag p has specified text
        cy.get(listIssue)
          .should("have.length", 5)
          .first()
          .find("p")
          .contains("TEST_TITLE")
          .siblings()
          .within(() => {
            //Assert that correct avatar and type icon are visible
            cy.get(pickleRickAvatar).should("be.visible");
            cy.get(storyIcon).should("be.visible");
          });
      });

    cy.get(boardListBackLog)
      .contains("TEST_TITLE")
      .within(() => {
        // Assert that correct avatar and type icon are visible
        cy.get(pickleRickAvatar).should("be.visible");
        cy.get(storyIcon).should("be.visible");
      });
  });

  it("Should validate title is required field if missing", () => {
    // System finds modal for creating issue and does next steps inside of it
    cy.get(modalIssueCreate).within(() => {
      // Try to click create issue button without filling any data
      clickingCreateIssueButton();

      // Assert that correct error message is visible
      cy.get('[data-testid="form-field:title"]').should(
        "contain",
        "This field is required"
      );
    });
  });

  it("Should create a custom issue and validate it successfully", () => {
    cy.get(modalIssueCreate).within(() => {
      cy.get(descriptionInputField).type("My Bug description");
      cy.get(descriptionInputField).should("have.text", "My Bug description");
      cy.get(inputNameTitle).type("Bug");
      cy.get(inputNameTitle).should("have.value", "Bug");
      selectTypeDropdown();
      cy.get(selectOptionBug).wait(1000).trigger("mouseover").trigger("click");
      cy.get(bugIcon).should("be.visible");
      selectPriority();
      cy.get(priorityOptionHighest)
        .wait(1000)
        .trigger("mouseover")
        .trigger("click");
      selectingReporterPicleRickDropdown();
      selectingAssigneeLordGabenDropdown();
      clickingCreateIssueButton();
    });

    cy.get(modalIssueCreate).should("not.exist");
    cy.contains(successMessage).should("be.visible");
    cy.reload();
    cy.contains(successMessage).should("not.exist");

    cy.get(boardListBackLog)
      .should("be.visible")
      .and("have.length", 1)
      .within(() => {
        // Assert that this list contains 5 issues and first element with tag p has specified text
        cy.get(listIssue)
          .should("have.length", 5)
          .first()
          .find("p")
          .contains("Bug")
          .siblings()
          .within(() => {
            cy.get(lordGabenAvatar).should("be.visible");
            cy.get(bugIcon).should("be.visible");
          });
      });
  });
});
