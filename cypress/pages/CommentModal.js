class CommentModal {
  ...
  editComment(originalComment, editedComment) {
    ...
    this.getCommentTextArea()
      ...
      .type(editedComment);
    this.clickSaveButton();
    this.getIssueComment().should('contain', editedComment);
  }

  deleteComment(commentText, isLastComment = false) {
    ...
    if (isLastComment) {
      this.getIssueComment().should('not.exist');
    } else {
      this.getIssueComment().should('exist').and('not.contain', commentText);
    }
  }
